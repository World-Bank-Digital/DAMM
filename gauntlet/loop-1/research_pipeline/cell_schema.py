...
def threshold_text(direction, thresholds, unit=""):
    ...
    return (f"SCORING: this is a THRESHOLD indicator ({d}), unit {unit or 'as published'}. "
            f"Level bands are set by the cut points {cuts}: a value below the first cut is "
            f"level 1, and each cut passed raises the level by one, to level 5 above the "
            f"last cut. Record the value; the level follows from it arithmetically, so "
            f"`rung_argument` should justify the VALUE and its construct match, not the "
            f"arithmetic.")
