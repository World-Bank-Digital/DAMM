#!/usr/bin/env python3
"""Measure what the historical automated challenge bought with its allocation.

The legacy input filenames contain ``_g2``. They record a machine-only challenge, not
DAMM human G2. The design record says plainly that whether the machine pass deserves
its protected share is unknown until the shadow figures can be re-run with it. This is
that arithmetic: the same comparison against the same verified assessment, before and
after, with the automated challenge's own cost beside it.
...
def main():
    ...
    findings = V.load_compatible_json_alias(
        os.path.join(LOOP1, f"{args.before}_automated_challenge_findings.json"),
        os.path.join(LOOP1, f"{args.before}_g2_findings.json"),
        "automated-challenge findings",
    ) or []
    challenge_ledger = V.load_compatible_json_alias(
        os.path.join(LOOP1, f"{args.before}_automated_challenge_spend.json"),
        os.path.join(LOOP1, f"{args.before}_g2_spend.json"),
        "automated-challenge spend ledgers",
    )
    spend = (challenge_ledger or {}).get("summary", {})
    ...
    L = []
    w = L.append
    w(f"# Did the automated challenge earn its share? — {args.country}\n")
    w(f"The same comparison against the same verified assessment, before and after the "
      f"machine challenge. `{args.before}` is the first pass alone; `{args.after}` is "
      f"the first pass with the automated challenge's surviving findings applied. This "
      f"machine pass does not satisfy human G1 or G2.\n")

    w("\n## What it cost\n")
    challenge_cost, fpc = spend.get("total", 0), first.get("total", 0)
    # A first-pass ledger smaller than the challenge ledger is not a cheap first pass. It
    ...
    w(f"| | first pass | automated challenge | challenge as a share |")
    w("|---|---|---|---|")
    w(f"| cost | ${fpc:.2f}{' (incomplete)' if truncated else ''} | "
      f"${challenge_cost:.2f} | "
      + ("not derivable from an incomplete first-pass ledger"
         if truncated else
         f"{100.0 * challenge_cost / max(fpc + challenge_cost, 1e-9):.0f}% "
         "of the two passes together") + " |")
    ...
    w(f"\nThe historical budget design reserved 15% of the country budget for the "
      f"automated challenge — $75 of $500. It spent **${challenge_cost:.2f}**, which is "
      f"{100.0 * challenge_cost / 75.0:.0f}% of that allocation.\n")
    ...
    for k in ("filled", "withdrawn", "relevelled", "adjusted", "upheld"):
        if k in outcomes:
            w(f"| {k} | {outcomes[k]} |")
    w(f"\n**{changed} rows** had their level changed by the automated challenge "
    ...
    w("| measure | before automated challenge | after | change |")
    w("|---|---|---|---|")
    ...
    if filled:
        w(f"\nGaps the automated challenge closed: {', '.join(filled)}.")
    if newheld:
        w(f"\nLevels the automated challenge withdrew: {', '.join(newheld)}.")
    ...
    w("**Moving toward the verified assessment is not the only good outcome.** The "
      "verified assessment is the best available answer, not the truth, and the "
      "automated challenge can be right where it disagrees with it — the hand-run "
      "gauntlet's machine challenge changed rows that had already passed an earlier "
      "human assessment. A row moved away from the verified level is a row to read, not "
      "a row to count against the challenge.\n")
    w("**Two independent runs of the first pass alone differ on about four rows.** A "
      ...
      "than that is evidence about the challenge rather than run-to-run variance.\n")

    out = os.path.join(LOOP1, f"AUTOMATED-CHALLENGE-VALUE-{args.before}.md")
    open(out, "w").write("\n".join(L) + "\n")

    print(f"automated-challenge cost ${challenge_cost:.2f} "
          f"({100.0 * challenge_cost / 75.0:.0f}% of its $75 allocation) "
