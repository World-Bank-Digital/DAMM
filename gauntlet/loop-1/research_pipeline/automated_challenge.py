#!/usr/bin/env python3
"""Automated evidence challenge for canonical Stage 1 (decision C5).

A different vendor attacks the first machine pass. Scoped to the rows where being
...
spots, so the challenger is drawn from a different vendor than the primary.

*Its own retrieval.* Query planning sees the construct and never the first pass's
sources, so the automated challenge searches from scratch. Handing it the same pack would
let it re-judge but never *find*, and finding what the first pass missed is where the
hand-run gauntlet's automated challenge earned its keep — four gap refutations came out
of nothing but different search phrasings.

*The same evidence gates.* An automated-challenge proposal is evidence like any other
and enters through `gates.run_gates` exactly as a first-pass answer does. A challenger
that could write a level straight into a row would be a bypass around quote verification
and the prerequisite bar, and the second opinion would have weaker rules than the first.

And one asymmetry, deliberately: **failing to find something is not a refutation.** If
the automated challenge cannot independently locate what the first pass recorded, that
is a provenance note, not a downgrade — the hand protocol's own rule, where a gap the
refuter cannot fill is confirmed rather than deepened.

Usage:
    python3 automated_challenge.py --country Egypt --iso EGY --run EGY_shadow \\
...
PASS = "automated_challenge"


# ------------------------------------------------------------------ schema

def _automated_challenge_schema():
...
AUTOMATED_CHALLENGE_SCHEMA = _automated_challenge_schema()
# Compatibility for imports from the pre-canonical machine pass. This is only a schema
# alias; it does not represent or satisfy human G2.
G2_SCHEMA = AUTOMATED_CHALLENGE_SCHEMA

AUTOMATED_CHALLENGE_SYSTEM = SYSTEM + """

YOU ARE AN AUTOMATED CHALLENGER, NOT A HUMAN ASSESSOR OR HUMAN G2 REVIEWER.
...
def challenge_row(spec, row, record, country, llm, ledger, prereq, gaps, holds, log):
    ...
    ans = llm.json_call(
        AUTOMATED_CHALLENGE_SYSTEM,
        user,
        AUTOMATED_CHALLENGE_SCHEMA,
        PASS,
        max_tokens=6000,
        detail=f"automated challenge {spec['id']}",
    )
...
def decide(spec, row, f):
    """Apply one machine-challenge finding. Return (outcome, replacement or None).

    Four outcomes, and the conservative one is the default:

      upheld          the row stands. Either the challenge confirmed it, or its attempt did
      ...
      filled          a gap or a withheld level is replaced by evidence that cleared
                      every gate. This is the automated challenge earning its keep.
      withdrawn       the challenge showed affirmatively that the cited source does not support
    ...
    # the challenger must have quote-verified something it actually read.
    #
    # Without that, the asymmetry above is enforced by the challenger's own labelling
    # rather than by evidence — and the challenger is the thing being checked. Both
    # countries' 7.12 withdrawals came from a challenger that asserted no value, produced
    ...
    #
    # The legitimate case survives. A challenger that reads a page and finds it does not
    ...
    if verdict == "refuted" and kind in ("source does not support the value",
        ...
        out["note"] = (f"RATIFICATION HOLD — withdrawn by automated challenge: {f['reason']} "
    ...
    if not row_has_level and proposed["level"] is not None:
        out = dict(proposed)
        out["note"] = (f"Recorded by automated challenge, which located evidence the first "
    ...
    if verdict in ("adjust", "refuted"):
        ...
        # challenger argued the rung was understated and the row kept its old one anyway.
        #
        ...
        # it replaces. Levels the challenge raised are listed separately in the report, because
        # raising a level is the direction that needs a reader.
        if proposed.get("level") is not None and proposed["level"] != row["level"]:
            out = dict(proposed)
            out["note"] = (f"Level {row['level']} to {proposed['level']} after automated "
                           f"challenge: {f['reason']} " + (proposed.get("note") or "")).strip()
            for k in ("defnote", "defsev"):
                if k in row:
                    out[k] = row[k]
        ...
        for k in ("src", "url", "tier", "tier_detail", "year"):
            if proposed.get(k):
                out[k] = proposed[k]
        out["note"] = (f"Provenance adjusted by automated challenge: {f['reason']} "
                       + (row.get("note") or "")).strip()
        return "adjusted", out
    return "upheld", None
...
def write_report(path, country, run, findings, counts, summ, scope_n, vendor):
    ...
    w(f"# Automated evidence challenge of {run} ({country})\n")
    w(f"A different vendor (`{vendor}`) attacked the first machine pass on the "
    ...
    w("| outcome | rows | |")
    w("|---|---|---|")
    w(f"| **filled** | {len(by('filled'))} | a gap or withheld level replaced by "
      "evidence that cleared every gate |")
    w(f"| **withdrawn** | {len(by('withdrawn'))} | a level removed: the cited source "
      "was shown not to support it |")
    w(f"| **relevelled** | {len(by('relevelled'))} | the level changed: a second "
      "reading of better evidence, cleared through every gate |")
    ...
    w("\n## Verdicts as the automated challenger gave them\n")
    w("| verdict | rows |")
    w("|---|---|")
    for k, v in sorted(counts["verdict"].items(), key=lambda kv: -kv[1]):
        w(f"| {k} | {v} |")
    w("\nA challenger verdict is not the same thing as a change to the row. A refutation "
    ...
    for outcome, heading, blurb in (
        ("filled", "Rows the automated challenge filled",
        ...
        ("withdrawn", "Levels the automated challenge withdrew",
        ...
        ("relevelled", "Levels the automated challenge changed",
        ...
        ("adjusted", "Provenance the automated challenge corrected",
    ...
    w("\n## Every row challenged\n")
    w("| id | role | challenger verdict | outcome | what it found |")
    w("|---|---|---|---|---|")
    ...
    w("\n## Reading this\n")
    w("The automated challenge is scoped to the rows the first pass was least sure of, "
      "so a high upheld count is not a null result — it is the first pass's abstentions "
      "being confirmed as real by an independent searcher, which is exactly what makes "
      "a recorded gap trustworthy. The figure that decides whether the challenge earns "
      "its share of the budget is **filled** plus **withdrawn**: rows where a second "
      "machine pass changed what the assessment says.\n")
...
def output_paths(run, *, legacy_g2_names=False):
    """Return canonical output names, or the retired names for the compatibility shim."""
    stem = "g2" if legacy_g2_names else "automated_challenge"
    report = (
        f"G2-REPORT-{run}.md"
        if legacy_g2_names
        else f"AUTOMATED-CHALLENGE-REPORT-{run}.md"
    )
    return {
        "state": os.path.join(LOOP1, f"{run}_{stem}_state.json"),
        "spend": os.path.join(LOOP1, f"{run}_{stem}_spend.json"),
        "findings": os.path.join(LOOP1, f"{run}_{stem}_findings.json"),
        "input": os.path.join(LOOP1, f"{run}_{stem}_input.json"),
        "report": os.path.join(LOOP1, report),
    }


def _select_legacy_resume_mode(run, *, resume, explicit_legacy):
    """Resume an old checkpoint without making legacy names canonical for new runs."""
    canonical = output_paths(run)
    legacy = output_paths(run, legacy_g2_names=True)
    checkpoint_keys = ("state", "spend", "findings", "input", "report")
    canonical_present = set()
    legacy_present = set()
    for key in checkpoint_keys:
        has_canonical, has_legacy = V.compatible_alias_presence(
            canonical[key],
            legacy[key],
            f"automated-challenge {key} checkpoints",
        )
        if has_canonical:
            canonical_present.add(key)
        if has_legacy:
            legacy_present.add(key)
    if not resume and (canonical_present or legacy_present):
        raise ValueError(
            "automated-challenge outputs already exist; use --resume or a new run id"
        )
    if canonical_present and legacy_present:
        if canonical_present != legacy_present:
            raise ValueError("mixed canonical and legacy automated-challenge checkpoints")
        if explicit_legacy:
            return True
        return False
    if explicit_legacy:
        if canonical_present and not legacy_present:
            raise ValueError(
                "canonical automated-challenge outputs already exist; "
                "the legacy entry point cannot create a parallel identity"
            )
        return True
    if not resume:
        return False
    return bool(legacy_present)


def _publish_canonical_outputs(paths, run):
    """After a successful legacy resume, publish immutable canonical aliases."""
    canonical = output_paths(run)
    payloads = {}
    for key in ("state", "spend", "findings", "input", "report"):
        V.compatible_alias_presence(
            canonical[key],
            paths[key],
            f"automated-challenge {key} outputs",
        )
        with open(paths[key], "rb") as handle:
            payloads[key] = handle.read()
    for key, content in payloads.items():
        V.publish_bytes_once(
            canonical[key],
            content,
            label=f"canonical automated-challenge {key}",
        )
...
def main(argv=None):
    ...
    ap.add_argument("--reapply", action="store_true",
                    help="recompute what each saved finding does to its row, and rewrite "
                         ...
                         "carry the challenger's answer, its gate results and its proposed "
                         "row, so a corrected decision rule applies to the challenge already "
                         "paid for rather than occasioning another one.")
    args = ap.parse_args(argv)

    legacy_mode = _select_legacy_resume_mode(
        args.run,
        resume=args.resume,
        explicit_legacy=args.legacy_g2_output_names,
    )
    paths = output_paths(args.run, legacy_g2_names=legacy_mode)
    publish_canonical = legacy_mode and not args.legacy_g2_output_names
    ...
    # A paid challenge can be journalled before its finding reaches the state file. Resume
    ...
    if loaded_state:
        print(f"resuming — {len(state['findings'])} rows already challenged, "
    ...
    if args.reapply:
        findings = json.load(open(paths["findings"]))
        for f in findings:
            spec = specs[f["id"]]
        ...
        return finish(
            ...
            prior_spend=json.load(open(paths["spend"])).get("summary"),
        )

    print(f"Automated challenge on {args.run} · challenger {vendor}/{llm.model}")
    print(f"scope: {len(scope)} of 57 rows — {len(prereq)} prerequisites, "
          f"{len(gaps)} gaps, {len(holds)} holds ({len(prereq & (gaps | holds))} overlap)")
    print(f"budget ${args.ceiling:.0f}, automated-challenge allocation "
          f"${ledger.cap(PASS):.0f}\n")
    ...
    def work(iid):
        ...
        t0 = time.time()
        try:
            f = challenge_row(specs[iid], rows[iid], records.get(iid), args.country,
    ...
    if stopped:
        print(f"\n!! {stopped}")
        print("   Rows never challenged are absent from the findings, NOT recorded as "
              "upheld — an unchallenged row must not read like one that survived.")
...
def finish(
    ...
    *,
    ...
    out_input = paths["input"]
    json.dump(corrected, open(out_input, "w"), indent=1, default=str)
    json.dump(findings, open(paths["findings"], "w"), indent=1, default=str)
    summ = prior_spend or ledger.summary()
    if not prior_spend:
        ledger.save(paths["spend"])
    write_report(
        paths["report"],
        args.country,
        args.run,
    ...
    if publish_canonical:
        _publish_canonical_outputs(paths, args.run)

    print(f"\nchallenged {len(findings)} rows · "
    ...
    print(f"wrote {os.path.basename(out_input)} and {os.path.basename(paths['report'])}")
    return 0
