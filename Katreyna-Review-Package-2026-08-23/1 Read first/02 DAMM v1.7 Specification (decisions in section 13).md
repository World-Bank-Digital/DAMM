...
**Quality control.** Three layers, specified in `DAMM-v1.7-QC-Protocol.md`: **structural** — the §3–§7 rules, enforced by construction; **automated** — render-time checks on consistency, provenance, reconciliation and presentation, where the renderer refuses to emit on failure and states the result in the report footer; and **three human gates** — G1 assessor confirmation of the machine pass · G2 peer review of every prerequisite row and every Judged row · G3 TTL sign-off against the four prohibitions before anything circulates.
...
| Automated QC | `qc_checks()` in the renderer — emit-blocking; result stated in the report footer; human-gate sign-offs recorded in the workbook Read Me at migration |
...
Workbook remains source of truth. The v1.7 pipeline is: machine pass → tiered research → assessor confirmation (G1) → engine → automated QC → report; corrections at every gate are first-class input files, so the whole chain reproduces from source.
...
9. **The QC Protocol** — confirm gate placement and the G2 scope, and note the amendment §14 records: peer review must ask whether evidence answers *this* indicator, not only whether the source states the number.
