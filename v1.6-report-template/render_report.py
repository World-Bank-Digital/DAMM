#!/usr/bin/env python3
"""DAMM v1.6 report renderer — reference implementation.

Reads the engine JSON (nigeria_v16.json) plus the initiative register
(register_<country>.json) and renders the ten-section Diagnostic Package
(spec §9) as a self-contained HTML report.

Two registers, one firewall: scored sections render only what the engine
computed from recorded values; researched-narrative blocks are visibly
distinct and never set a level.
"""
import json, html, sys, os
from collections import Counter

HERE = os.path.dirname(os.path.abspath(__file__))
DATA = os.path.join(HERE, "..", "v1.6-test-run", "nigeria_v16.json")
REG  = os.path.join(HERE, "register_nigeria.json")
OUT  = os.path.join(HERE, "Nigeria-DAR-Diagnostic.html")
...
def tierbadge(t):
    names = {"T1":"Tier 1 — official statistics / IO databases","T2":"Tier 2 — peer-reviewed / IO flagship",
             "T3":"Tier 3 — government legal & policy artefacts","T4":"Tier 4 — reputable grey literature",
...
def pillar_chart():
    ...
    for i,p in enumerate(PILLAR_ORDER):
        ...
        s.append(f'<text x="{X(pd["mean"])+8:.1f}" y="{y+17}" class="vallab">{pd["mean"]:.2f}&#8194;{bandtxt}</text>')
    s.append('</svg>')
    return "".join(s)
...
def evidence_chart():
    ...
    for i,p in enumerate(PILLAR_ORDER):
        ...
        s.append(f'<text x="{x0-12}" y="{y+16}" class="rowlab" text-anchor="end">{p} <tspan class="dimlab">n={n}</tspan></text>')
        x = x0
        for key in ("Measured","Documented","Judged","Gap"):
            ...
            s.append(f'<rect x="{x:.1f}" y="{y+3}" width="{max(w,2):.1f}" height="18" rx="2" class="seg seg-{key.lower()}">'
                     f'<title>{esc(PILLAR_NAMES[p])}: {c} {key}</title></rect>')
            if w > 16:
...
REFS = {
  "1.2": "world \u2248 4,100 kg/ha \u00b7 FAOSTAT 2023",
  "8.1": "world 9.1% \u00b7 FAO SOFI 2024",
}

# ---------- automated QC (Layer 2, DAMM-v1.6-QC-Protocol.md; emit-blocking) ----------
def qc_checks(d, reg):
    ...
    add("A5 counts reconcile to 57; matrix has 6 columns",
        dict(Counter(v["cls"] for v in inds.values())) == d["counts"]
    ...
    add("A7 register tiers valid; T5 carries no results tier",
        ...
        and all(not e["results_tier"] for e in reg["entries"] if e["tier"] == "T5"))
...
sentence = (f'Foundations and policy read <em>Established</em> (C1 2.75, C3 2.86, A1 2.60); the enabling core is '
            f'<em>Emerging</em> — and in {", ".join(weak_pillars)} the band renders parenthesized: on current evidence, '
            f'those are findings about the assessment as much as the country. The machine-first pass recorded values for '
            f'{valuebacked} of {N} indicators and named {counts["Gap"]} gaps; nothing here rests on an unrecorded claim.')

matrix_counts = Counter(m["status"] for m in d["matrix"].values())

# Strategic questions — derived per spec §9 rules
SQ = [
 ("Blocked cell AGI \u00b7 3.11",
  "Agricultural data interoperability standards are Absent (3.11, L1). Which institution gets the standards mandate, and what moves 3.11 to Adopted before \u2014 not after \u2014 NIDAP scales?",
  ("India\u2019s AgriStack established national agricultural data standards and farmer-registry rails on digital ID \u2014 operating at national scale.", "T3")),
 ("Blocked cell AGI \u00b7 4.5",
  "No agricultural data governance framework exists (4.5, Absent) while AI-enabled public agro-systems launch. Who owns farm-level data, under what consent regime, given responsible-AI safeguards are Present but narrow (7.12)?",
  None),
 ("Prerequisite 3.3 \u00b7 register overlap",
  "GES and ABP enrolled millions in now-closed silos; the national registry holds ~279k (<1% of farming households). Consolidate, federate, or rebuild \u2014 what is the registry path that FIN and AGI both depend on?",
  ("Rwanda\u2019s Smart Nkunganire consolidated input-subsidy registration onto one national platform.", "T4")),
 ("Universal prerequisite 2.9",
  "Rural electricity access is 23.5% \u2014 Present, narrowly. Which use cases can sequence inside today\u2019s rural power envelope, and where must electrification lead before digital service rollout?",
  ("Pay-as-you-go solar bundled with agricultural services is documented at scale across East and West Africa.", "T4")),
 ("Partial cell ADV \u00b7 5.5",
  "Digital extension capability is Present (narrow): platforms exist, the extension ratio does not support them. What pairing of community-based advisor (CBA) models and digital advisory moves ADV from Partial to Ready?",
  ("Digital Green\u2019s video-mediated extension carries independent impact evaluations; Ethiopia\u2019s 8028 farmer hotline operates nationally.", "T2")),
 ("Unverified prerequisite 5.7",
  "Does FMAFS have a staffed, funded digital/AI unit? The desk pass could not verify it (5.7) \u2014 a first-mission question, because both delivery-risk flags hang on it.",
  None),
 ("Matrix \u00b7 thin enablers",
  "Every non-blocked column is Partial for the same reason \u2014 thin enablers (layer mean 2.32 vs Foundation 2.67). Which two or three enabler investments unlock the most columns at once?",
  None),
 ("Evidence ledger",
  f"{counts['Gap']} of {N} indicators are recorded gaps, including five of A1\u2019s ten need metrics (post-harvest loss, market access, credit, climate-smart agriculture adoption, women\u2019s landholding). What primary data collection must the mission commission first?",
  None),
]

IMPACT = [
 ("1.2", "Cereal yield 1,558 kg/ha — less than half of irrigated-system benchmarks", ["ADV","SMF","AGI"]),
 ("8.1", "Undernourishment 19.9% of population", ["ADV","FIN","MKT"]),
 ("1.1", "Agriculture value added per worker $3,495 with 33.5% of employment in agriculture", ["SMF","FIN"]),
 ("1.5", "Post-harvest loss rate — recorded gap", ["SCM","MKT"]),
 ("1.6", "Smallholder access to formal markets — recorded gap", ["MKT"]),
 ("1.7", "Agricultural credit access — recorded gap", ["FIN"]),
 ("1.8", "Climate-smart practice adoption — recorded gap", ["ADV","AGI"]),
 ("8.5", "Women's landholding share — recorded gap", ["ADV","SMF","MKT","SCM","FIN","AGI"]),
]
...
def a1_tiles():
    ...
    for k in ["1.1","1.2","1.3","1.4","8.1"]:
        ...
        if isinstance(val, float) and abs(val) >= 100:
            ...
            disp = f'{val:g}' if isinstance(val, float) else f'{val}'
        ...
        tiles.append(f'<div class="tile"><div class="tile-k">{esc(v["name"])}{stale}</div>'
                     ...
                     + f'<div class="tile-m">{chip(v["cls"])} L{v["level"]} · {esc(v["src"].split(";")[0])} · {v["year"]}</div></div>')
    gaps = [k for k in ["1.5","1.6","1.7","1.8","8.5"]]
    gl = "".join(f'<li>{esc(d["indicators"][k]["name"])} <span class="dim">({k})</span></li>' for k in gaps)
    tiles.append(f'<div class="tile tile-gap"><div class="tile-k">Recorded gaps — 5 of 10 need metrics</div><ul class="gaplist">{gl}</ul>'
                 f'<div class="tile-m">{chip("Gap")} searched, named, routed to the ledger (§9)</div></div>')
    return (f'<div class="tiles">{"".join(tiles)}</div>'
            '<p class="caption">Reference values (world) are shown where compiled \u2014 context only, never scored; the full peer-set reference column lands at workbook migration.</p>')
...
def register_table():
    ...
    for e in reg["entries"]:
        ...
        res = esc(e["results"]) + (f' {tierbadge(e["results_tier"])}' if e["results_tier"] else "")
        rows.append(f'<tr><td class="rn">{esc(e["name"])}<div class="dim">{esc(e["lead"])}</div></td>'
                    ...
                    f'<td>{tierbadge(e["tier"])}<div class="dim srcline">{esc(e["src"])}</div></td></tr>')
...
def matrix_table():
    ...
    return ('<div class="tablewrap"><table class="tbl matrix"><thead><tr><th></th>'+head+'</tr></thead><tbody>'
            ...
            f'<tr><td class="rowhd">Mean enabler level</td>{mean}</tr>'
            '</tbody></table></div>')
...
def kpi_table():
    rows = "".join(f'<tr><td class="num">{esc(k["id"])}</td><td>{esc(k["name"])}</td>'
                   f'<td class="num">{k["value"]:,g}</td><td class="num">{k["year"]}</td><td>{esc(k["src"])} {tierbadge(tier_of(k["src"]))}</td>'
...
def appendix():
    ...
    for k, v in d["indicators"].items():
        ...
        rows.append(f'<tr class="cls-{v["cls"].lower()}"><td class="num">{esc(k)}</td><td>{esc(v["name"])}</td>'
                    ...
                    f'<td>{esc(v["src"])} {tierbadge(t)}</td></tr>')
...
@media print{
  body{background:#fff}
  .fbar{display:none}
  section{break-inside:avoid-page;margin-top:34px}
...
<div class="how"><h4>Evidence classes</h4><p><span class="chip chip-measured">M</span> Measured &#8212; a number was recorded &nbsp;<span class="chip chip-documented">D</span> Documented &#8212; a citable artefact &nbsp;<span class="chip chip-judged">J</span> Judged &#8212; assessor statement without artefact &nbsp;<span class="chip chip-gap">G</span> Gap &#8212; searched for and not found. The class is derived from the recorded value, never chosen.</p></div>
<div class="how"><h4>Levels &amp; bands</h4><p>Every indicator carries a level, L1&#8211;L5. A pillar band is the plain mean of its levels: Nascent &lt;1.8 &#8226; Emerging &lt;2.6 &#8226; Established &lt;3.4 &#8226; Advanced &lt;4.2 &#8226; Transformative &#8805;4.2. A band in (parentheses) rests mostly on judgement or gaps.</p></div>
...
<div class="how"><h4>Source tiers</h4><p>T1 official statistics &amp; international databases &#8226; T2 peer-reviewed &amp; flagship reports &#8226; T3 government legal and policy artefacts &#8226; T4 reputable grey literature &#8226; T5 news/vendor material (admitted only in the register, for existence facts). Tiers are reported, never weighted.</p></div>
...
narr_problem = f"""<div class="narrative"><p class="narrlab">Researched narrative · desk draft — cites scored rows; sets no level</p>
<p>Nigeria&#8217;s need profile is a productivity story before it is a technology story. A third of the workforce
(33.5%, <b>1.3</b>) produces cereal yields of 1,558&#8202;kg/ha (<b>1.2</b>) — less than half of what irrigated systems in the
region demonstrate — while one person in five is undernourished (<b>8.1</b>). The food production index (<b>1.4</b>) reads
well but is stale (2022) and routed for refresh. The desk pass could not verify five of the ten need metrics —
post-harvest loss, market access, credit access, climate-smart adoption, and women&#8217;s landholding — which are
recorded as gaps and drive the ledger (§9), not silently omitted. Two A1 additions are proposed to the census
(spec §13.2): cereal import dependency and irrigation coverage, both machine-fetchable.</p></div>"""

narr_register = f"""<div class="narrative"><p class="narrlab">Researched narrative · desk draft — cites scored rows; sets no level</p>
<p>The landscape is crowded and discontinuous: the two largest programs (GES, ABP) are closed, the national
digital-agriculture strategy has waited unadopted since 2020, and NIDAP — the harmonization vehicle — is early.
{esc(reg["overlap_finding"])}</p></div>"""

brief = f"""<div class="brief"><p class="sentence">{sentence}</p>
<div class="brieftiles">
 <div class="btile"><div class="k">Strongest pillar</div><div class="v">C3 · 2.86</div><div class="m">Policy &amp; safeguards — Established</div></div>
 <div class="btile"><div class="k">Weakest pillar</div><div class="v">C4 · 2.00</div><div class="m">People &amp; institutions — (Emerging), weak evidence</div></div>
 <div class="btile"><div class="k">Use-case readiness</div><div class="v">{matrix_counts.get("Ready",0)} · {matrix_counts.get("Partial",0)} · {matrix_counts.get("Blocked",0)}</div><div class="m">Ready · Partial · Blocked — AGI (agricultural intelligence) blocked by interoperability standards (3.11) + data governance (4.5)</div></div>
 <div class="btile"><div class="k">Evidence base</div><div class="v">{valuebacked}/{N}</div><div class="m">value-backed (M+D) · {counts["Gap"]} recorded gaps · 3 stale</div></div>
</div></div>"""

html_out = f"""<meta charset="utf-8">
<title>Nigeria Digital Agriculture Diagnostic</title>
...
<header class="mast">
  <div class="kicker">Digital Agriculture Maturity Model · <b>DAMM v1.6</b> · Diagnostic Package</div>
  <h1>Nigeria</h1>
  <p class="subtitle">Digital agriculture diagnostic — the starting document for the National Digital Agriculture Roadmap (2025–2035).</p>
  <div class="metarow">
    <span>Assessment: <b>machine-first desk pass</b></span>
    <span>Engine: <b>v1.6 reference</b></span>
    <span>Data pass: <b>21 Aug 2026</b></span>
  ...
  Machine-first pass; {counts["Gap"]} of {N} indicators are recorded gaps and 8 rest on judgement. No level exists without a recorded value;
...
{sec(1, "Country problem statement", "Playbook 1A",
     ...
     "A1 is scored as need, not digital maturity — a low reading is a large opportunity. Pillar mean 2.60, Established, on the five value-backed rows.")}

{sec(2, "Initiative & solutions register", "Playbook 1B",
     register_table()
     + f'<p class="caption">{esc(reg["status_note"])} Status uses the presence ladder (Announced / Adopted / Operating / Closed). T5 rows evidence existence only; scale figures marked &#8220;vendor-reported&#8221; are claims, not findings. Register rows may serve as Documented evidence for E1/C4 indicators — cited by row.</p>'
     + narr_register,
...
{sec(3, "Pillar profile", "Playbook 1C",
     pillar_chart()
     + '<p class="caption">Band cut-offs at 1.8 / 2.6 / 3.4 / 4.2 (half-open). Hatched bars render parenthesized — Judged + Gap outnumber Measured + Documented, so the band is a finding about the assessment as much as the country.</p>'
     + evidence_chart() + legend_html
     + '<p class="caption">Evidence composition per pillar. C2, C4 and O1 carry the weak-evidence rendering; E1 escapes it by one row.</p>')}
...
{sec(5, "Use-case readiness matrix", "Playbook 2B — Digital Readiness",
     ...
     "Cells: Ready / Partial / Blocked (named blocker) / Unverified. Prerequisites bind on presence only — a fact, never an opinion. Delivery-risk flags sit on the cover and block nothing.")}

{sec(6, "Impact side", "Playbook 2B — Impact",
     impact_table()
     + '<p class="caption">Mapping: standard v1.6 crosswalk, curated by the task team leader (TTL). Constraints recorded as gaps still map — an unmeasured constraint is an evidence task (§9), not a reason to drop the use case.</p>')}
...
{sec(8, "Strategic questions", "Step 2 agenda",
     questions_list()
     + '<p class="caption">Derived mechanically from \u00a75 and \u00a77 under spec \u00a79 rules; task team leader (TTL) curation pending. Precedent pointers are existence proofs drawn from the Global Practice Library companion (DAMM-v1.6-Practice-Library-Schema.md) \u2014 relevance is selected by the readiness matrix; never endorsements, never country comparisons. Authored additions would be marked as such.</p>')}
...
<div class="method">
<h3>Method — DAMM v1.6 in five lines</h3>
<ol>
<li>Every indicator gets a value, a source, and a year — or a recorded gap. No level without a recorded value.</li>
<li>The value sets the evidence class: number → Measured · citable artefact → Documented · neither → Judged · looked-for-and-not-found → Gap. Derived, never chosen.</li>
...
<p class="qcline"><b>Automated QC:</b> {len(QC)}/{len(QC)} renderer checks passed (Layer 2, DAMM-v1.6-QC-Protocol.md); a failed check blocks the render. Human gates \u2014 G1 machine-pass confirmation, G2 peer review of prerequisite and Judged rows, G3 TTL sign-off \u2014 <b>pending</b>: this is a desk pass.</p>\n<p class="prohib">Prohibitions: no cross-country ranking · no band as a project development objective (PDO), disbursement-linked indicator (DLI), or disbursement condition · no automatic financing decisions ·
no public claim before human review. Rendered from <code>nigeria_v16.json</code> (engine v1.6 reference) + <code>register_nigeria.json</code> by <code>render_report.py</code> —
the same pipeline renders any country&#8217;s workbook. Tier badges auto-derived via the starter lookup in DAMM-v1.6-Source-Tier-Protocol.md.</p>
{GLOSS}
</div>
